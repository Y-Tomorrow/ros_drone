...
static mavros_msgs::PositionTarget makeBodyVelSetpoint(const Eigen::Vector4f& vxyzy) {
  ...
  // 注意：FRAME_BODY_NED 下，z 为向下为正。TargetSearcher 输出 vz_cmd 沿用 body 速度源，
  // 这里按“原样转发”做最小干预；如果你模型里 z 符号相反，可在此统一取反。
  pt.velocity.x = vxyzy.x();
  pt.velocity.y = vxyzy.y();
  pt.velocity.z = vxyzy.z();
  pt.yaw_rate = vxyzy.w();
  return pt;
}
...
static void toUpperAscii(std::string* s) {
  for (char& c : *s) {
...
static CameraID camFromString(const std::string& cam) {
  if (cam == "TELE") {
    return CameraID::TELE;
  }
  if (cam == "BOTH") {
...
static bool readInteractiveCamPixel(std::string* lost_from_cam, double* lost_pixel_x, double* lost_pixel_y) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    std::string err;
    if (parseCamPixelLine(line, lost_from_cam, lost_pixel_x, lost_pixel_y, &err)) {
      return true;
    }
    std::cerr << "[错误] " << err << "\n> " << std::flush;
  }
...
int main(int argc, char** argv) {
  ...
  ros::NodeHandle pnh("~");
  {
    ...
    if (!nh_wide.hasParam("enable_task1_decel")) {
      ROS_WARN(
          ...
          "请使用: roslaunch ros_drone target_searcher_tester.launch");
    }
  }
  ...
  CameraID cam = CameraID::WIDE;
  if (!session_mode) {
    if (interactive) {
      if (!readInteractiveCamPixel(&lost_from_cam, &lost_pixel_x, &lost_pixel_y)) {
        ROS_ERROR("未读到有效输入，退出。");
        return 1;
      }
    } else {
      toUpperAscii(&lost_from_cam);
    }
    cam = camFromString(lost_from_cam);
  } else {
    ROS_INFO("session_mode=true：起飞后在初始点悬停；终端输入任务，return 回原点，可多次执行。");
  }

  const std::string ns = uav_prefix;
  const std::string ns_mavros = uav_prefix + "/mavros";

  ROS_INFO("target_searcher_tester:");
  ...
  if (!session_mode) {
    ROS_INFO("  单次模式 lost_from_cam=%s lost_pixel=(%.2f, %.2f)", lost_from_cam.c_str(), lost_pixel_x, lost_pixel_y);
  ...
  if (session_mode) {
    ...
    CameraID run_cam = CameraID::WIDE;

    ROS_INFO(
        ">>> 悬停就绪。命令:  WIDE|TELE|BOTH <px> <py>  |  return / home / r  |  quit / q  |  help / h");
    ...
    while (ros::ok()) {
      ...
      if (!cmd_in.empty() && state == SessionState::IDLE_HOVER) {
        ...
        if (!tl.empty()) {
          if (tl == "quit" || tl == "exit" || tl == "q") {
            ...
            ROS_INFO(
                "WIDE|TELE|BOTH <px> <py> — 执行搜索，最长 %.1f s；return/home/r — 回初始悬停点；quit — 退出",
                scenario_duration_s);
            ...
            double py = 0.0;
            std::string err;
            if (parseCamPixelLine(t, &cam_s, &px, &py, &err)) {
              run_cam = camFromString(cam_s);
              searcher.reset();
              searcher.set_search_strategy(run_cam, Eigen::Vector2f(static_cast<float>(px), static_cast<float>(py)));
              state = SessionState::RUN_SEARCH;
              t_search0 = ros::Time::now().toSec();
              t_last = t_search0;
              ROS_INFO("执行任务 %s (%.2f, %.2f)，最长 %.1f s", cam_s.c_str(), px, py, scenario_duration_s);
            } else {
      ...
      switch (state) {
        ...
        case SessionState::RUN_SEARCH: {
          ...
          if (send_velocity) {
            pub_raw.publish(makeBodyVelSetpoint(out.vel_vxyzy));
          }
          if (send_gimbal) {
    ...
    ROS_INFO("target_searcher_tester finished.");
    return 0;
  }

  // =============== 单次模式：一条任务后退出或 hold ===============
  searcher.set_search_strategy(cam, Eigen::Vector2f(static_cast<float>(lost_pixel_x), static_cast<float>(lost_pixel_y)));
